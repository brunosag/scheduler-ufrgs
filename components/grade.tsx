...
import { cn, isSelected, selectTurma } from '@/lib/utils';
import { Cadeira, DataContext, DataContextType, Turma } from '@/context/data-context';
                  ...
                  <TableCell key={dia} className="border-l">
                    <div className="flex flex-col items-center gap-1.5">
                      {turmas.map(
                        (turma) =>
                          !(
                            showOnlySelected && !isSelected({ turma: turma.turma, cadeira: turma.cadeira, cadeiras })
                          ) &&
                          ...
                          turma.horario === horario.start && (
                            <span
                              ...
                              className={cn(
                                isSelected({ turma: turma.turma, cadeira: turma.cadeira, cadeiras })
                                  ? ''
                                  : 'opacity-25',
                                'flex items-center gap-1.5'
                              )}
                              onClick={() => selectTurma({ turma, cadeiras, setCadeiras })}
                            >
                              {turma.cadeira}
                              <Badge
                                variant="secondary"
