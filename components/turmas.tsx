...
export default function Turmas({ className }: { className?: string }) {
  const { horarios, dias, cadeiras, setCadeiras, turmas, setTurmas, updateSelectedTurma, localStorageReady } =
          ...
          <TableBody>
            {turmas.map((turma) => (
              <TableRow
                ...
                role="button"
                className={cn(
                  !isSelected({ turma: turma.turma, cadeira: turma.cadeira, cadeiras }) && 'opacity-30'
                )}
              >
