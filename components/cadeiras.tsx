...
export default function Cadeiras({ className }: { className?: string }) {
  const { cadeiras, setCadeiras, turmas, setTurmas, localStorageReady } = useContext(DataContext) as DataContextType;
  ...
  function handleDelete(cadeira: Cadeira) {
    ...
    const filteredTurmas = turmas.filter((item) => item.cadeira !== cadeira.name);
